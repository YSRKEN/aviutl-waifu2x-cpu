...
int main(int argc, char *argv[]){
			...
			for(auto k = 0; k < w2.size(); ++k){
				...
				for(auto kw = 0; kw < w3.size(); ++kw){
					...
					for(auto kh = 0; kh < f.size(); ++kh){
						float weight = stod(f[kh].to_str());
						fout.write(reinterpret_cast<const char*>(&weight), sizeof(float));
					}
				}
			}
		}
	...
	for(auto i = 0; i < a.size(); ++i){
		...
		for(auto j = 0; j < b.size(); ++j){
			float bias = stod(b[j].to_str());
			fout.write(reinterpret_cast<const char*>(&bias), sizeof(float));
		}
	}
}
